...
export default function ToyDetailPage() {
  ...
  const [item, setItem] = useState<ToyItem | null>(null)
  ...
  const fetchToyItem = async () => {
    try {
      setIsLoading(true)
      const response = await fetch(`/api/products/toys/${id}`)
      if (!response.ok) throw new Error("Failed to fetch")

      const data = await response.json()
      setItem(data.data)
    } catch (error) {
      console.error("Error fetching toy item:", error)
      toast.error("Failed to load product details")
      router.push("/products/toy")
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const addToCart = () => {
    ...
    try {
      ...
      if (existingIndex >= 0) {
        ...
        cartData.push({ ...item, quantity, categoryTypeModel: "toy" })
      ...
      <div className="min-h-screen relative">
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <Button onClick={() => router.push("/products/toy")}>Back to Toys</Button>
          </div>
        </div>
      </div>
    )
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  {discount > 0 && (
                    <Badge className="bg-green-100 text-green-800 text-lg px-2 py-1">
                      Save ₹{(item.actualPrice || 0) - item.sellingPrice}
                    </Badge>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Button
                  ...
                  size="lg"
                  className="flex-1 bg-white border-2 border-orange-500 text-orange-600 hover:bg-orange-50"
                >
              ...
              <Card className="border-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-lg">
                <CardContent className="p-6">
                  ...
                  <p className="text-muted-foreground leading-relaxed">
                    {item.description ||
                      "This amazing toy is designed to provide hours of fun and entertainment for children. Made with high-quality materials and attention to detail, it's perfect for developing creativity and imagination."}
                  </p>
                </CardContent>
              </Card>
            </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm">100% Safe Materials</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Truck className="h-5 w-5 text-blue-600" />
