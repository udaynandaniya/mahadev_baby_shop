...
export default function ClothesPage() {
  ...
  const fetchClothes = useCallback(
    async (page = 1, reset = false) => {
      try {
        ...
        if (filters.ageGroup.length > 0) params.append("ageGroup", filters.ageGroup.join(","))
        ...
        const response = await fetch(`/api/products/clothes?${params}`)
        ...
        if (reset || page === 1) {
          ...
          setClothes((prev) => [...prev, ...(data.data || [])])
        }
  ...
  const toggleWishlist = (item: ClothesItem) => {
    try {
      const savedWishlist = localStorage.getItem("wishlist")
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold bg-gradient-to-r from-pink-600 via-purple-600 to-blue-600 dark:from-pink-400 dark:via-purple-400 dark:to-blue-400 bg-clip-text text-transparent flex items-center justify-center gap-3">
              <Shirt className="h-8 w-8 md:h-10 md:w-10 text-pink-500" />
              Baby Clothes Collection
            </h1>
            <p className="text-muted-foreground mt-2 text-lg">
              Discover adorable and comfortable clothes for your little ones
            </p>
          </div>
          ...
          <Card className="border-0 bg-white/60 dark:bg-gray-900/60 backdrop-blur-sm shadow-lg mb-8">
            <CardContent className="p-4 space-y-4">
              ...
              <div className="relative">
                ...
                <Input
                  placeholder="Search clothes by name, color, or description..."
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                {/* Results Count */}
                <div className="text-sm text-muted-foreground">Showing {clothes.length} products</div>
              </div>
            </CardContent>
          </Card>
              ...
              return (
                ...
                >
                  <div className="relative aspect-[3/4] overflow-hidden">
                    <Link href={`/products/clothes/${item._id}`}>
                      <Image
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {item.category.map((cat) => (
                        <Badge key={cat} variant="outline" className="text-xs capitalize border-pink-200 text-pink-600">
                          {cat}
                        </Badge>
                      ))}
                      <Badge variant="secondary" className="text-xs bg-purple-100 text-purple-600">
                        {item.ageGroup[0]} years
                      </Badge>
                    </div>
          ...
          {clothes.length === 0 && !isLoading && (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-600 mb-2">No clothes found</h3>
