...
export default function ClothesDetailPage() {
  ...
  const fetchClothesItem = async () => {
    try {
      setIsLoading(true)
      const response = await fetch(`/api/products/clothes/${id}`)
      ...
      <div className="min-h-screen relative">
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen">
          <div className="text-center">
            ...
            <Button onClick={() => router.push("/products/clothes")}>Back to Clothes</Button>
          </div>
        </div>
      </div>
    )
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {item.category.map((cat) => (
                    <Badge key={cat} variant="outline" className="capitalize border-pink-200 text-pink-600">
                      {cat}
                    </Badge>
                  ))}
                  {item.ageGroup.map((age) => (
                    <Badge key={age} variant="secondary" className="bg-purple-100 text-purple-600">
                      {age} years
                    </Badge>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                ...
                <div>
                  ...
                  <p
                    className={`font-semibold ${item.inStock > 10 ? "text-green-600" : item.inStock > 0 ? "text-orange-600" : "text-red-600"}`}
                  >
              ...
              <Card className="border-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-lg">
                <CardContent className="p-6">
                  ...
                  <p className="text-muted-foreground leading-relaxed">
                    {item.description ||
                      "This beautiful baby clothing item is made with the finest materials to ensure comfort and style for your little one. Perfect for everyday wear and special occasions."}
                  </p>
                </CardContent>
              </Card>
            </div>
